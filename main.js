(function () {
  ...
  function fixOffsets() {
    ...
    const a =
      alertbar && !alertbar.classList.contains("hide")
        ? alertbar.offsetHeight || 52
        : 0;
    ...
    if (drawer) {
      ...
      drawer.style.height = `calc(100vh - ${top})`;
    }
    if (scrim) {
      scrim.style.top = top;
      scrim.style.height = `calc(100vh - ${top})`;
    }
  }
  ...
  (function () {
    ...
    function onScroll() {
      ...
      lastY = y;
      const now = Date.now();
      if (dy > 20 && y > 140 && !hidden && now - lastToggle > 420) {
      ...
      if (dy < -20 && hidden && now - lastToggle > 420) {
  ...
  function wireLangRadios() {
    ...
    function apply(lang) {
      ...
      I18N_APPLY(lang);
      const isEn = lang === "en";
      [hdr.en, hdr.ta, mob.en, mob.ta].forEach((el) => {
        if (!el) return;
        el.checked = el.value === "en" ? isEn : !isEn;
      });
    }
    [hdr.en, hdr.ta, mob.en, mob.ta].forEach((el) => {
      if (!el) return;
      el.addEventListener("change", () => apply(el.value));
      el.addEventListener("click", () => apply(el.value));
    });
  }
  ...
  function svcControls() {
    ...
    document
      .getElementById("svcExpandAll")
      ?.addEventListener("click", expandAll);
    document
      .getElementById("svcCollapseAll")
  ...
  (function () {
    ...
    form.addEventListener("submit", function (ev) {
      ...
      if (services.length === 0) {
        toast("Please select at least one service.", "error", 6000);
        return;
      }
      if (!name) {
        toast("Name is required.", "error", 6000);
        return;
      }
      if (!phone || !/^\d{10}$/.test(phone)) {
        toast("Phone number must be 10 digits.", "error", 6000);
        return;
      }
      if (!vehicle) {
        toast("Vehicle (Make/Model) is required.", "error", 6000);
        return;
      }
      const now = new Date();
      if (date) {
        ...
        if (d < today) {
          toast("Please choose a future date.", "error", 6000);
          return;
        }
        if (d.getTime() === today.getTime()) {
          if (!time) {
            toast("Please choose a time.", "error", 6000);
            return;
          }
          ...
          if (selected <= now) {
            toast("Time must be in the future for today.", "error", 6000);
            return;
          }
        }
      }
